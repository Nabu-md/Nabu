...
// RAG mode keeps the Safe file surface (no shell) and leans on the Nabu MCP
// `search_notes_semantic` tool for retrieval.
const CLAUDE_RAG_AGENT_TOOLS: &str = "Read,Edit,MultiEdit,Write,Glob,Grep,LS";
...
fn mcp_config(vault_path: &str, vault_paths: &[String]) -> Result<String, String> {
    ...
    let config = serde_json::json!({
        "mcpServers": {
            "nabu": crate::cli_agent_runtime::nabu_node_mcp_server(
                &mcp_server_path,
                vault_path,
                vault_paths,
                false,
...
fn agent_tools(permission_mode: AiAgentPermissionMode) -> &'static str {
    match permission_mode {
        ...
        AiAgentPermissionMode::Rag => CLAUDE_RAG_AGENT_TOOLS,
    }
}
...
fn preapproved_agent_tools(permission_mode: AiAgentPermissionMode) -> Option<&'static str> {
    match permission_mode {
        ...
        AiAgentPermissionMode::Rag => None,
    }
}
...
fn disallowed_agent_tools_compat(permission_mode: AiAgentPermissionMode) -> &'static str {
    match permission_mode {
        ...
        AiAgentPermissionMode::Rag => CLAUDE_SAFE_DISALLOWED_TOOLS_COMPAT,
    }
}
...
mod tests {
    ...
    fn mcp_config_is_valid_json() {
        ...
        if let Ok(config_str) = mcp_config("/tmp/test-vault", &extra_vaults) {
            ...
            assert!(parsed["mcpServers"]["nabu"]["command"].is_string());
            assert_eq!(
                parsed["mcpServers"]["nabu"]["env"]["VAULT_PATH"],
                "/tmp/test-vault"
            );
            assert_eq!(
                parsed["mcpServers"]["nabu"]["env"]["VAULT_PATHS"],
