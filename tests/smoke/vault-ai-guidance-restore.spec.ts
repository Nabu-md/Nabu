...
test('vault guidance restore command recovers missing managed guidance', async ({ page }) => {
  ...
  await expect(aiBadge).toHaveAttribute('title', /Nabu guidance missing or broken/)

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Nabu AI Guidance')).toBe(true)
  await page.keyboard.press('Enter')

  await expect(page.getByText('Nabu AI guidance restored')).toBeVisible()

  await aiBadge.click()
  await expect(page.getByTestId('status-ai-guidance-summary')).toHaveText('Nabu guidance ready')
  await expect(page.getByTestId('status-ai-guidance-restore')).toHaveCount(0)
  await page.keyboard.press('Escape')

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Nabu AI Guidance')).toBe(false)
})
