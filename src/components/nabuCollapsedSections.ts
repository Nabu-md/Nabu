...
import {
  ...
  type NabuBlockNoteEditor,
} from './nabuBlockNoteDom'
...
const headingCollapseStores = new WeakMap<NabuBlockNoteEditor, CollapsedHeadingStore>()
...
function collapsedHeadingStore(editor: NabuBlockNoteEditor) {
...
export function useCollapsedHeadingIds(editor: NabuBlockNoteEditor) {
...
function collapsedSectionStyleScope(editorElement: HTMLElement) {
  ...
  container.dataset.nabuCollapseScope ??= String(++collapsedSectionScopeSequence)
  return `[data-nabu-collapse-scope=${cssString(container.dataset.nabuCollapseScope)}]`
}
...
function collapsedSectionStyleElement(editorElement: HTMLElement) {
  ...
  styleElement.setAttribute('data-nabu-collapsed-sections', 'true')
...
function collapsedHeadingHoverRuleSelectors(
  ...
  return Array.from(renderState.collapsedHeadingIds)
    .flatMap((blockId) => headingDotsSelectorsForStyle(
      editorElement,
      blockId,
      `${scope}[data-nabu-collapse-hover-id=${cssString(blockId)}]`,
    ))
}
...
export function collapsedSectionHiddenBlockIds(editor: NabuBlockNoteEditor): ReadonlySet<string> {
  const store = collapsedHeadingStore(editor)
...
function ensureCollapsedHeadingRenderer(
  ...
  const setHoveredDotsHit = (hit?: CollapsedHeadingDotsHit) => {
    ...
    const container = collapsedSectionContainer(editorElement)
    if (container) {
      if (hit) container.dataset.nabuCollapseHoverId = hit.blockId
      else delete container.dataset.nabuCollapseHoverId
    }
...
function releaseCurrentCollapsedHeadingRenderer(editor: NabuBlockNoteEditor) {
...
export function useCollapsedHeadingRendering(editor: NabuBlockNoteEditor) {
  useLayoutEffect(() => {
    const store = collapsedHeadingStore(editor)
