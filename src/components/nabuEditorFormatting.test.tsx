...
describe('nabuEditorFormatting', () => {
  it('keeps the markdown-safe toolbar controls and block type select', () => {
    const itemKeys = filterNabuFormattingToolbarItems(
      getFormattingToolbarItems(getNabuBlockTypeSelectItems()),
  ...
  it('returns the audited markdown-safe block types for the toolbar select', () => {
    expect(getNabuBlockTypeSelectItems()).toEqual([
  ...
  it('filters unsupported toggle slash-menu variants and removes command descriptions', () => {
    ...
    const items = filterNabuSlashMenuItems([
    ...
    ] satisfies NabuSlashMenuTestItem[])
  ...
  it('wraps slash-menu icons so hover can swap Phosphor weights', () => {
    ...
    >
    expect(icon.props.className).toBe('nabu-slash-menu-icon')
    expect(iconChildren.map((child) => child.props.className)).toEqual([
      'nabu-slash-menu-icon__regular',
      'nabu-slash-menu-icon__fill',
    ])
  ...
  it('keeps custom media slash-menu commands searchable', () => {
    ...
    const items = filterNabuSlashMenuItems(expectedCommands.map((item) => ({
      ...item,
      onItemClick: () => {},
    })))
