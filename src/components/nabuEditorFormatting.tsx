...
import {
  filterNabuFormattingToolbarItems,
  getNabuBlockTypeSelectItems,
} from './nabuEditorFormattingConfig'
...
type NabuFloatingOptions = NonNullable<FloatingUIOptions['useFloatingOptions']>
type NabuFloatingMiddleware = NonNullable<NabuFloatingOptions['middleware']>[number]
...
const TOLARIA_BASIC_TEXT_STYLE_TOOLTIPS = {
  ...
  Exclude<NabuBasicTextStyle, typeof MARKDOWN_HIGHLIGHT_STYLE>,
...
} satisfies Record<NabuBasicTextStyle, PhosphorIcon>

type NabuSelectedBlock = ReturnType<
  BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>['getTextCursorPosition']
>['block']

type NabuSelectedFileBlock = {
  type: string
  url: string
}
...
function isNabuSelectedBlock(value: unknown): value is NabuSelectedBlock {
...
function nabuSelectedBlocks(value: unknown): NabuSelectedBlock[] {
  return Array.isArray(value) ? value.filter(isNabuSelectedBlock) : []
}
...
type NabuBlockTypeSelectOption = ReturnType<
  typeof getNabuBlockTypeSelectItems
>[number] & {
...
function viewportClampMiddleware(): NabuFloatingMiddleware {
  return {
    name: 'nabuViewportClamp',
...
function getSelectedBlocksSafely(
  ...
  try {
    const selectionBlocks = nabuSelectedBlocks(editor.getSelection()?.blocks)
  ...
  try {
    ...
    return isNabuSelectedBlock(block) ? [block] : []
  } catch {
    return []
  }
}
...
function getCursorBlockSafely(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
): NabuSelectedBlock | null {
  try {
    const block = editor.getTextCursorPosition().block
    return isNabuSelectedBlock(block) ? block : null
  } catch {
    return null
  }
}
...
function getBasicTextStyleButtonState(
  basicTextStyle: NabuBasicTextStyle,
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
) {
...
function getBlockTypeItemIconElement(
  item: ReturnType<typeof getNabuBlockTypeSelectItems>[number],
) {
...
function isSelectedBlockTypeItem(
  item: ReturnType<typeof getNabuBlockTypeSelectItems>[number],
  firstSelectedBlock: NabuSelectedBlock,
) {
...
function getNabuBlockTypeSelectOptions(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  firstSelectedBlock: NabuSelectedBlock,
) {
  return getNabuBlockTypeSelectItems()
    .filter((item) =>
      editorHasBlockWithType(
        editor,
        item.type,
...
function NabuBasicTextStyleButton({
  basicTextStyle,
  locale = 'en',
}: {
  basicTextStyle: NabuBasicTextStyle
...
function NabuBlockTypeSelect() {
  ...
  const selectedBlocks = useEditorState({
    editor,
    selector: ({ editor }): NabuSelectedBlock[] => getSelectedBlocksSafely(editor),
  })
  const firstSelectedBlock = selectedBlocks[0] ?? null
  const selectItems = useMemo(
    () => (
      firstSelectedBlock
        ? getNabuBlockTypeSelectOptions(editor, firstSelectedBlock)
        : []
    ),
    [editor, firstSelectedBlock],
  )
  const selectedItem = selectItems.find(
    (item): item is NabuBlockTypeSelectOption => item.isSelected,
  )
  ...
  const handleBlockTypeChange = useCallback((item: NabuBlockTypeSelectOption) => {
...
function NabuFileDownloadButton({ vaultPath }: { vaultPath?: string }) {
  const Components = useRequiredComponentsContext()
  ...
  return items.flatMap((item) => {
    switch (String(item.key)) {
      case 'blockTypeSelect':
        return [<NabuBlockTypeSelect key={item.key} />]
      case 'boldStyleButton':
        return [<NabuBasicTextStyleButton basicTextStyle="bold" key={item.key} />]
      case 'italicStyleButton':
        return [<NabuBasicTextStyleButton basicTextStyle="italic" key={item.key} />]
      case 'strikeStyleButton':
        return [<NabuBasicTextStyleButton basicTextStyle="strike" key={item.key} />]
      case 'fileDownloadButton':
        return [<NabuFileDownloadButton key={item.key} vaultPath={vaultPath} />]
      default:
        return [item]
    }
  })
...
function insertExtraTextStyleButtons(items: ReactElement[], locale: AppLocale) {
  ...
  return [
    ...
    <NabuBasicTextStyleButton basicTextStyle="code" key="codeStyleButton" />,
    <NabuBasicTextStyleButton
...
function getNabuFormattingToolbarItems(vaultPath: string | undefined, locale: AppLocale) {
...
export function NabuFormattingToolbar({
  locale = 'en',
  vaultPath,
}: {
  locale?: AppLocale
  ...
  return <FormattingToolbar>{getNabuFormattingToolbarItems(vaultPath, locale)}</FormattingToolbar>
}

type NabuFormattingToolbarControllerProps = {
      ...
      {shouldRender && (
        ...
        >
          <BlockTypeMenuContext.Provider value={blockTypeMenuState}>
            {Component ? <Component /> : <NabuFormattingToolbar />}
...
export function NabuFormattingToolbarController(props: NabuFormattingToolbarControllerProps) {
  const editor = useBlockNoteEditor<
    BlockSchema,
    InlineContentSchema,
    StyleSchema
