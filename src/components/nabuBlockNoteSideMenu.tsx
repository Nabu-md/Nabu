...
import { usePointerBlockReorder } from './nabuBlockReorder'
import { useSideMenuTextAlignment } from './nabuSideMenuAlignment'
import {
  blockHeadingLevel,
...
} from './nabuCollapsedSections'
import {
  liveSideMenuBlock,
  runSideMenuAction,
  type SideMenuBlock,
} from './nabuSideMenuBlocks'
import { turnBlockIntoType } from './richEditorBlockTypeCommands'
import {
  createNabuSlashMenuIcon,
...
type NabuSideMenuProps = SideMenuProps & {
  locale?: AppLocale
}
...
function NabuHeadingCollapseButton({ locale }: { locale: AppLocale }) {
  const Components = useRequiredComponentsContext()
  const { block, editor } = useSideMenuBlock()
...
function NabuSectionControlButton({ locale }: { locale: AppLocale }) {
  const { block, editor } = useSideMenuBlock()
  if (isCollapsibleSectionBlockForEditor(editor, block)) return <NabuHeadingCollapseButton locale={locale} />

  return <NabuAddBlockButton />
}

function NabuDragHandleButton({
  children,
  dragHandleMenu,
  locale = 'en',
  ...
  return (
    ...
    >
      <Components.Generic.Menu.Trigger>
        <span
          className="nabu-block-drag-handle"
      ...
      {dragHandleMenu
        ...
        : <NabuDragHandleMenu locale={locale}>{children}</NabuDragHandleMenu>}
    </Components.Generic.Menu.Root>
  )
}

function NabuRemoveBlockItem({ children }: { children: ReactNode }) {
  const Components = useRequiredComponentsContext()
  const { block, editor } = useSideMenuBlock()
...
function NabuTurnBlockIntoSubmenu({ locale }: { locale: AppLocale }) {
  const Components = useRequiredComponentsContext()
  const { block, editor } = useSideMenuBlock()
  ...
  return (
    <Components.Generic.Menu.Root sub position="right">
      ...
      <Components.Generic.Menu.Dropdown className="nabu-turn-into-menu-dropdown" sub>
        {getNabuBlockTypeSelectItems().map((item) => (
          <Components.Generic.Menu.Item
            key={item.key}
            className="bn-menu-item"
            icon={createNabuSlashMenuIcon(item.icon)}
            onClick={() => {
              runSideMenuAction(() => {
...
function NabuDragHandleMenu({
  ...
  return (
    <DragHandleMenu>
      {children}
      <NabuRemoveBlockItem>{dict.drag_handle.delete_menuitem}</NabuRemoveBlockItem>
      <NabuTurnBlockIntoSubmenu locale={locale} />
      <NabuTableHeaderItem header="row">{dict.drag_handle.header_row_menuitem}</NabuTableHeaderItem>
      <NabuTableHeaderItem header="column">{dict.drag_handle.header_column_menuitem}</NabuTableHeaderItem>
    </DragHandleMenu>
  )
}

export function NabuSideMenu({ locale = 'en', ...props }: NabuSideMenuProps) {
  const { block, editor } = useSideMenuBlock()
  useSideMenuTextAlignment(editor, block)

  return (
    <SideMenu {...props}>
      <NabuDragHandleButton locale={locale} />
      <NabuSectionControlButton locale={locale} />
    </SideMenu>
  )
}

export function NabuCollapsedHeadingsController() {
  const editor = useBlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>()
