...
export type NabuSlashMenuItem = DefaultReactSuggestionItem & {
  key: string
  submenuItems?: NabuSlashMenuItem[]
}
type NabuBlockTypeSelectItem = RichEditorBlockTypeDefinition & {
  icon: PhosphorIcon
}
...
export function createMathSlashMenuItem(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  labels: Pick<NabuSlashMenuLabels, 'mathTitle'> = { mathTitle: 'Math' },
): NabuSlashMenuItem {
  return createBlockSlashMenuItem(editor, {
    key: 'math',
...
export function createSandboxBlockSlashMenuItem(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  labels: Pick<NabuSlashMenuLabels, 'sandboxBlockTitle'> = {
...
export function createCalloutSlashMenuItem(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  labels: Pick<NabuSlashMenuLabels, 'calloutTitle' | 'calloutTypeTitles'> = {
...
export function addItemsToMediaGroup(
  items: NabuSlashMenuItem[],
  mediaItems: NabuSlashMenuItem[],
): NabuSlashMenuItem[] {
...
export function createNabuSlashMenuIcon(Icon: PhosphorIcon) {
  return createElement(
    'span',
    { className: 'nabu-slash-menu-icon' },
    createElement(Icon, {
      'aria-hidden': true,
      className: 'nabu-slash-menu-icon__regular',
      size: 18,
      weight: 'regular',
    }),
    createElement(Icon, {
      'aria-hidden': true,
      className: 'nabu-slash-menu-icon__fill',
      size: 18,
      weight: 'fill',
    }),
  )
...
export function filterNabuFormattingToolbarItems<T extends ReactElement>(
  items: T[],
): T[] {
  return items.filter(
...
export function filterNabuSlashMenuItems<T extends NabuSlashMenuItem>(
  items: T[],
): T[] {
  return items
    ...
    .map((item) => {
      ...
      return {
        ...item,
        icon: NabuIcon ? createNabuSlashMenuIcon(NabuIcon) : item.icon,
        subtext: undefined,
      }
    }) as T[]
}

export function getNabuSlashMenuItems(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  query: string,
  labels?: NabuSlashMenuLabels,
) {
  const defaultItems = getDefaultReactSlashMenuItems(editor) as NabuSlashMenuItem[]
