...
declare global {
  interface Window {
    __nabuEditorMemoryProbe?: EditorMemoryProbeApi
  }
}
...
function useEditorMemoryProbeBridge(api: EditorMemoryProbeApi): void {
  useEffect(() => {
    ...
    window.__nabuEditorMemoryProbe = api
    return () => {
      if (window.__nabuEditorMemoryProbe?.run === api.run) {
        delete window.__nabuEditorMemoryProbe
      }
    }
  }, [api])
}
