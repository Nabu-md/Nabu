...
Nabu notes are durable Markdown files, while whiteboard editing needs an interactive canvas with structured shape data. tldraw provides a mature React whiteboard runtime and exposes snapshot APIs that can persist the document without using browser-local `persistenceKey` storage.

The storage decision needs to preserve Nabu's filesystem source-of-truth rule: deleting local caches must not lose a board, raw mode must expose the canonical source, and Git should track the board together with the surrounding note.

## Decision

**Nabu will support whiteboards as Markdown-durable fenced `tldraw` blocks backed by tldraw document snapshots.**
...
- Debounces tldraw document changes into BlockNote block props so Nabu's normal autosave writes the snapshot into the `.md` file.
...
- **Markdown fenced block with tldraw document snapshot** (chosen): keeps the board in the note file, matches Nabu's Mermaid/math round-trip model, and works for both full whiteboard notes and embedded boards inside larger notes.
