...
Nabu notes are local Markdown files, and users need a deliberate way to place small static HTML surfaces inside a note without giving that markup Nabu, Tauri, filesystem, or vault privileges. Plain fenced `html` code blocks are portable and familiar, but rendering them directly in the editor DOM would create XSS, navigation, and credential-exposure risks.
...
**Nabu treats fenced `html` blocks as Markdown-durable `htmlBlock` nodes rendered inside sanitized, opaque-origin sandboxed iframes.** The portable storage format is a fenced `html` block with an explicit optional `height` attribute, and scripts/forms/same-origin/top-navigation privileges are not granted to the iframe.
...
- **Render sanitized HTML directly in the editor DOM**: simpler and easier to style, but still shares origin and DOM with Nabu and is too easy to regress into app-privileged execution.
...
- Resizing persists by serializing `height="..."` on the `html` fence; if the metadata is missing or invalid, Nabu uses the default block height.
...
- External links are rewritten for a new browsing context with `noreferrer noopener`; future native-open integration can supersede this if Nabu needs stricter system-browser routing from inside iframe content.
