...
Nabu identifies type definitions by markdown frontmatter (`type: Type`), not by filesystem location. Older documentation and UI creation flows still treated `type/` as the canonical destination for new type documents, with a compatibility fallback for vaults that already used `types/`.
...
**Nabu creates new type documents at the vault root.**
...
- Root filename collisions are handled as file collisions; Nabu must not overwrite an existing note when creating a type document.
