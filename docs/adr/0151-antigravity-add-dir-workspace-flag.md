...
Nabu still needs app-managed Antigravity runs to start in the active vault, expose that vault as the only intended workspace, and preserve the Safe / Power User permission mapping from ADR-0103.
...
The subprocess `current_dir` remains the active vault path, and Nabu still writes the transient MCP config to `<vault>/.agents/mcp_config.json` before launch.
...
Nabu still avoids `--dangerously-skip-permissions`.

## Consequences

- Antigravity CLI versions that reject `--cwd` can start from the Nabu AI panel.
