...
- passes Nabu MCP as a per-session `--additional-mcp-config` JSON payload instead of mutating `~/.copilot/mcp-config.json`
- uses Safe mode with `--available-tools=write,nabu`, `--allow-tool=write,nabu`, and `--deny-tool=shell`
...
- Users can select GitHub Copilot anywhere Nabu lists local/app-managed AI agents.
- App-managed Copilot sessions stay vault-scoped by current working directory and Copilot's default path verification unless the user chooses Nabu Power User, which still does not bypass all paths or URLs.
- Nabu does not persist Copilot credentials, model-provider secrets, or durable third-party MCP files.
