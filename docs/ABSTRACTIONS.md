...
Key abstractions and domain models in Nabu.

## Design Philosophy

Nabu's abstractions follow the **convention over configuration** principle: standard field names, types, and relationships have well-defined meanings and trigger UI behavior automatically. This makes vaults legible both to humans and to AI agents — the more a vault follows conventions, the less custom configuration an AI needs to navigate it correctly.
...
These frontmatter field names have special meaning in Nabu's UI:
...
Relationship fields are detected dynamically — any frontmatter field containing `[[wikilink]]` values is treated as a relationship (see [ADR-0010](adr/0010-dynamic-wikilink-relationship-detection.md)). Nabu's own default relationship vocabulary uses snake_case on disk, but labels are humanized at render time and existing user-authored keys are left untouched.
...
- It is used by Nabu internally for configuration, behavior, and UI preferences
...
Git initialization is intentionally scoped to dedicated vault folders. When the current non-git folder looks like a broad personal root such as Documents, Desktop, or Downloads and does not already carry Nabu-managed vault markers, `init_git_repo` refuses to run Git and asks the user to select or create a dedicated subfolder instead.
...
|---|---|---|
| `markdown` or absent | `.md`, `.markdown` | Full Nabu note model: frontmatter, BlockNote, raw editor, relationships, title sync |
...
Asset previewability is inferred in the renderer from the filename extension (`src/utils/filePreview.ts`) rather than stored as a new persisted kind. Supported images render through `<img>`, supported audio/video render through native HTML media controls, and supported PDFs render through the webview's PDF object renderer, all backed by Tauri asset URLs. Packaged PDF previews require both `object-src` and `frame-src` to allow the scoped Tauri asset sources because the webview PDF renderer can load the document in a nested frame context. HEIC/HEIF is intentionally handled as an unsupported image import format until Nabu has a deterministic cross-platform decoder/conversion path; Windows webviews cannot be assumed to decode iPhone HEIC files, so picker uploads and native drops report a localized fallback message and do not save a broken attachment. On Linux AppImage builds, `should_use_external_media_preview` can disable in-webview audio/video rendering so the same file blocks show filename/external-open fallback controls instead of triggering unstable WebKitGTK media playback. Runtime asset access is accumulated only for vault roots Nabu has loaded in the current app session, because Tauri directory forbids cannot be safely reversed after a vault switch. The "open in default app" action re-enters the active-vault command boundary through `open_vault_file_external` before delegating to the native opener. This keeps the filesystem as source of truth and avoids converting assets into proprietary objects.

Standalone `.html` and `.htm` vault entries load as ordinary text tabs, but default to `HtmlFilePreview` instead of BlockNote or CodeMirror. `htmlFilePreviewSrcDoc` sanitizes the whole document, installs a restrictive Content Security Policy, removes remote passive loads, and rewrites only vault-contained local resource paths to Tauri asset URLs. The preview iframe has an opaque origin and no script or same-origin sandbox capability, so its document cannot access Nabu's DOM, storage, or Tauri IPC. The existing raw-source preference is shared with Markdown: the breadcrumb source action or `Cmd/Ctrl+\` swaps the preview for CodeMirror, and the external-open command remains the escape hatch for browser-only behavior. The breadcrumb derives action availability from the entry kind: standalone HTML keeps source/preview, file location, link, PDF export, git diff, and delete actions while omitting Markdown metadata, neighborhood, reading-width, table-of-contents, and archive controls.
...
A Markdown note with `_display: sheet` displays in the dedicated sheet editor instead of BlockNote. `type` remains semantic metadata, so a sheet can still be a `Project`, `Responsibility`, or any other Nabu type. The note remains a plain-text file: frontmatter stores ordinary Nabu properties plus `_sheet` presentation metadata, while the body stores CSV-like rows containing cell inputs and formulas.
...
├── AGENTS.md              ← canonical Nabu AI guidance
...
New notes are created at the vault root: `{vault}/{slug}.md`. Changing a note's type only requires updating the `type:` field in frontmatter — the file does not move. Moving a note into a user folder is a separate filesystem concern: the folder path changes, but the note keeps the same filename and `type:` value. Legacy `type/` and `types/` folders are still scanned like other non-hidden vault folders, so existing type documents in those folders continue to work, but new type documents created by Nabu are written at the vault root. Legacy `config/` content is still recognized during migration and repair, but Nabu's managed AI guidance now lives at the vault root.

A `flatten_vault` migration command is available to move existing notes from type-based subfolders to the vault root.

### Types as Files

Each entity type can have a corresponding **type document**: any markdown note with `type: Type` in its frontmatter. Nabu creates new type documents at the vault root (e.g., `project.md`, `person.md`) and still reads existing type documents from subfolders. Type documents:
...
Nabu separates **display title** from the file identifier:
...
Collections are the renderer-side foundation for note groups plus their presentation configuration. A collection can come from a built-in filter, type section, folder, saved View, or Neighborhood mode. Product terminology can stay simple: users select a collection and Nabu presents it.
...
`SidebarSelection.kind === 'entity'` is Nabu's Neighborhood mode for note-list browsing.
...
| `author.rs` | Author identity | Resolves the exact commit author Nabu will use, heals the legacy Nabu fallback email, and reports when repo-local identity shadows the global Git identity |
...
- Nabu keeps `defaultLanguage: "text"` so unlabeled code blocks do not silently become JavaScript at creation time. Parsed unlabeled code blocks then run through Nabu's lightweight language inference, while explicit fence languages and user dropdown choices still win.
...
Defined in `src/utils/calloutCatalog.ts`, `src/utils/calloutMarkdown.ts`, `src/utils/richEditorMarkdown.ts`, `src/utils/editorDurableMarkdown.ts`, `src/components/CalloutBlock.tsx`, `src/components/calloutIcons.ts`, `src/components/NabuSlashMenu.tsx`, `src/components/nabuEditorFormattingConfig.ts`, `src/components/editorSchema.tsx`, and styled in `src/components/Editor.css`:
...
- All default Obsidian types and aliases map to Nabu's semantic color families and a distinct Phosphor icon per canonical type; unknown type tokens retain neutral note styling and still serialize unchanged. Callouts use a borderless tinted surface with a larger title.
...
- Raw CodeMirror mode always shows the plain Markdown source, so imported technical notes stay editable outside Nabu.
...
- The rich editor renders the block with the `tldraw` package and saves debounced document snapshot changes back into the block props, so normal Nabu autosave writes the board into the `.md` file.
...
- The tldraw runtime receives Nabu's resolved light/dark mode as its user color scheme, so embedded whiteboards follow the app appearance and update while mounted.
...
- The iframe is a pointer preview, not a nested keyboard scope: it stays out of the tab order and releases accidental iframe focus back to the rich editor so Escape and Nabu shortcuts remain app-owned. The toolbar and resize controls remain normal focusable controls.
...
Defined in `src/components/nabuEditorFormatting.tsx` and `src/components/nabuEditorFormattingConfig.ts`:

- `SingleEditorView` disables BlockNote's default formatting toolbar, `/` menu, side menu, and file panel, then mounts Nabu-owned controllers so the visible formatting surface matches Nabu's markdown round-trip guarantees. `NabuFilePanel` reuses BlockNote's existing file/image UI only while its target block still exists; note reloads and untitled renames therefore close a stale attachment panel instead of rendering it against missing block state.
...
- The `/` slash menu remains the supported path for markdown-safe block transformations such as headings, quotes, list blocks, Mermaid diagrams, sandboxed HTML blocks, and whiteboards. `/date`, `/time`, and `/datetime` resolve the user's current local calendar values to plain Markdown text (`YYYY-MM-DD`, `HH:mm`, and `YYYY-MM-DD HH:mm`) at insertion time. Nabu filters out BlockNote's toggle-heading and toggle-list variants because those do not map cleanly to the markdown note model.
- The block-handle side menu keeps only actions that survive Nabu's markdown round-trip. Delete and table-header toggles remain available; BlockNote's `Colors` submenu is removed because block colors are not part of Nabu's supported markdown surface. Nabu renders the add-block button outside the drag handle so the handle stays next to the block content. `nabuSideMenuAlignment.ts` aligns the side menu to the first rendered text line for the hovered block, so H1/H2 typography, line-height, wrapping, and theme changes do not need per-heading offsets. `nabuBlockReorder.ts` owns block reordering as a pointer gesture with direct BlockNote block moves instead of HTML5 `DataTransfer`, keeping it independent from Tauri's native file-drop system. `nabuSideMenuBlocks.ts` owns stale-block lookup and mutation guards, so block-handle actions re-resolve the current live BlockNote block before mutating or dragging and note reloads/sync churn cannot leave controls acting on stale block references.
...
The app uses internal light and dark themes owned by Nabu, with System as an installation-local preference that follows the OS appearance (see [ADR-0081](adr/0081-internal-light-dark-theme-runtime.md) and [ADR-0112](adr/0112-system-theme-mode.md)). The previous vault-authored theming system remains removed.
...
Installation-local layout state that should not sync through a vault stays in localStorage. `useLayoutPanels` stores the clamped sidebar, note-list, and inspector widths under `nabu:layout-panels` so pane sizing survives app relaunches on the same machine.

### AI Guidance Files

Nabu tracks managed vault-level AI guidance separately from normal note content:
- `AGENTS.md` is the canonical managed guidance file for Nabu-aware coding agents
...
- `restore_vault_ai_guidance` repairs only Nabu-managed files and creates the optional Antigravity/Gemini shim on explicit request; user-authored custom `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` files are surfaced as custom and left untouched
...
Vault guidance is intentionally short and vault-specific. General Nabu product behavior is delivered through the bundled agent docs resource instead:
...
- `get_agent_docs_path` exposes the resolved resource folder to the renderer, and `buildAgentSystemPrompt()` tells every app-managed CLI agent to read vault `AGENTS.md` first, then search the bundled docs for Nabu behavior
...
- User can create a new empty vault, open an existing folder, or clone the public Getting Started vault into a chosen parent folder; Nabu derives the final `Getting Started` child path before cloning
- After the starter repo clone completes, Nabu removes every remote so the new vault opens local-only by default
- Welcome state tracked in localStorage (`nabu_welcome_dismissed`, with legacy fallback)
...
Nabu delegates remote auth to the user's system git setup:
...
- On Linux AppImage launches, every system-git command and MCP runtime subprocess (Node.js or Bun) removes AppImage loader overrides such as `LD_LIBRARY_PATH`, `LD_PRELOAD`, and `GIT_EXEC_PATH` before spawning, so helpers like `git-remote-https` and the system MCP runtime bind against the host library stack instead of Nabu's bundled WebKit/AppImage libraries
...
- On Linux AppImage launches, release packaging bundles the GTK3 fcitx immodule into the AppImage and startup environment safeguards write a cache-local `GTK_IM_MODULE_FILE` that points GTK at the mounted module whenever fcitx is configured. If the user has not explicitly chosen a GTK IM module, Nabu also sets `GTK_IM_MODULE=fcitx`, allowing WebKitGTK editor input to reach fcitx5 on both Wayland and X11 fallback launches without relying on host GTK module cache paths.
